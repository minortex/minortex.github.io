...
在 C++ 中，想实现一个闭包，还是得定义一个普通函数（虽然说跟一般的匿名函数印象不太一样）：
...
在 C++23 之前，如果定义类的时候使用 `const`，那么就需要添加一个成员重载函数：
...
类型转换重载也是 `operator` 操作符，不过重载的目标其实有点像 C 的强制转换：
...
A 可改，B 不可改。那么就定义一个类型转换的重载。

A 不可改，B 可改。那么就定义一个参数只含 A 的转换构造函数，让 B 去接收 A。

A、B 都不可改。那么定义一个普通函数实现转换吧。
